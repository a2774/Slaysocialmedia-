...
import { onClearGlobalCart } from "@/store/reducers/userSlice";
...
const DomainList = () => {
  ...
  const fetchDomains = async (
    ...
    try {
      const response = await axiosInstance.get(
        `${BASE_URL}/freename/domainsOwned?type=SECOND_LEVEL_DOMAIN&page=${page}&limit=${limit}&name=${searchQuery}`
      );
  ...
  useEffect(() => {
    ...
    // if (currentPage >= 1) {
    if (currentPage === 0) {
      fetchDomains(1, rowsPerPage, debouncedSearchTerm);
    } else fetchDomains(currentPage, rowsPerPage, debouncedSearchTerm);
    // }
  }, [currentPage, rowsPerPage, debouncedSearchTerm]);

  // Debounce mechanism: Update debouncedSearchTerm 500ms after user stops typing
  useEffect(() => {
    ...
    try {
      ...
      if (response.data) {
        ...
        if (router.query.token || router.query.PayerID) {
          ...
          // fetchDomains(currentPage, rowsPerPage, debouncedSearchTerm);
  ...
  console.log("##vurrent", currentPage, rowsPerPage, domainsList.length);
  return (
    <Layout>
